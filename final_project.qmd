...
employment_trends_clean <- employment_trends %>%
  ...
  select(-c(DGUID, SYMBOL, TERMINATED, SCALAR_FACTOR, SCALAR_ID, STATUS, 
            DECIMALS, Estimate, VECTOR, COORDINATE, UOM_ID)) %>%
  rename(
...
kable(summary_stats, 
      caption = "Summary Statistics for Employment Trends in Canada") %>%
...
ggplot(industry_trends_canada, aes(x = Date, y = Total_Employment, 
...
ggplot(goods_summary, aes(x = Industry, y = Average_Employment, fill = Period)) +
  ...
  scale_fill_manual(values = c("Pre-Pandemic" = "#2C7FB8", 
                               "Post-Pandemic" = "#D95F02")) +
  scale_y_continuous(labels = scales::comma)
```
...
## First Hypothesis Test: Does the mean employment level in goods producing industries in Canada between the pre-pandemic and post-pandemic periods have a statistically significant difference?
...
H_1 = \mu_{1} \neq \mu_{2}
$$ 
where mu represents the average total employment during a specific time period (1 = pre-pandemic and 2 = post-pandemic.) After setting our hypotheses, we have to calculate the test statistic using the formula 
...
$$
We get our result of 76.55 for degrees freedom, and then calculate the p-value.
```{r}
pt(4.64, 76.55, lower.tail = FALSE)
```
